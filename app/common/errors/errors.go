...
func NewAPIError(code int, status string, detail string) *APIError {
	if status == "" {
...
func NewFullAPIError(id string, code int, status string, detail string, xRequestId string) *APIError {
	return &APIError{
		Id:         id,
		Code:       code,
		Status:     status,
...
func ConvertFrom(err error) *APIError {
	apiErr := NewAPIError(http.StatusInternalServerError, "GENERAL_INTERNAL_SERVER_ERROR", err.Error())
...
func NewError(apiError *APIError, format string, a ...interface{}) *APIError {
	apiError.Detail += fmt.Sprintf(format, a...)
	return apiError
}
...
var (
	...
	ErrQueryParameterFault = &APIError{
		...
		Status: "Query_Invalid_Param",
		Detail: "Query parameter invalid",
	}
	...
	ErrMobileNumberIncorrect = &APIError{
		Code:   http.StatusBadRequest,
		Status: "Mobile_Number_Format_Incorrect",
		Detail: "Mobile number format is incorrect",
	}

	ErrTelephoneNumberIncorrect = &APIError{
		Code:   http.StatusBadRequest,
		Status: "Telephone_Number_Format_Incorrect",
		Detail: "Telephone number format is incorrect",
	}

	ErrImTokenGetFail = &APIError{
		Code:   http.StatusInternalServerError,
		Status: "Internal_Server_Error",
		Detail: "Failed to get the im token from the im server",
	}

	ErrClaimMissing = &APIError{
		Code:   http.StatusUnauthorized,
		Status: "UNAUTHORIZED",
		Detail: "Required claims are missing in the token",
	}

	ErrTokenGenErr = &APIError{
		Code:   http.StatusInternalServerError,
		Status: "Internal_Server_Error",
		Detail: "Error happened when generating the access token",
	}

	ErrHTTPRequestFail = &APIError{
		Code:   http.StatusBadRequest,
		Status: "HTTP_Request_Fail",
		Detail: "HTTP request fail",
	}
	ErrDifferentTenant = &APIError{
		Code:   http.StatusForbidden,
		Status: "Different_Tenant",
		Detail: "Is not the same tenant",
	}

	// S3
	ErrS3GetBucketInfo = &APIError{
		Code:   http.StatusBadRequest,
		Status: "S3_Get_Bucket_Info",
		Detail: "Get s3 bucket info failed",
	}
	ErrS3GetSecurityTokenFaild = &APIError{
		Code:   http.StatusBadRequest,
		Status: "S3_Get_Security_Token",
		Detail: "Get s3 security token failed",
	}
	ErrS3CreateObject = &APIError{
		Code:   http.StatusBadRequest,
		Status: "S3_Create_Object",
		Detail: "Create s3 object failed",
	}
	ErrS3DeleteObject = &APIError{
		Code:   http.StatusBadRequest,
		Status: "S3_Delete_Object",
		Detail: "Delete s3 object failed",
	}

	ErrResourceTotalCountGetFail = &APIError{
		Code:   http.StatusInternalServerError,
		Status: "Resource_Total_Count_Get_Fail",
		Detail: "Failed to get the total count for the resouce",
	}
...
func (e *APIError) Response() gin.H {
	return gin.H{"id": e.Id, "code": e.Code, "status": e.Status, "detail": e.Detail, "x_request_id": e.XRequestId}
}
...
//	T := i18n.T(lang)
//	return NewFullAPIError(e.Code, e.Status, T(e.Detail), copyNestedErrors(e.Errors), e.XRequestId)
//}
//
func (e APIError) Copy4LangWithRequestId(lang string, requestId string) *APIError {
	T := i18n.T(lang)
	return NewFullAPIError(e.Id, e.Code, e.Status, T(e.Detail), requestId)
}
